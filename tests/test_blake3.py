"""BLAKE3-focused Python binding coverage.

Symmetric counterpart to test_blake2s.py / test_blake2b.py: the
same coverage shape (nonce-size sweep, single + triple roundtrip,
single + triple auth with tamper rejection, persistence sweep,
plaintext-size sweep) applied to blake3. BLAKE3 ships at a single
width (-256) — there is no -512 BLAKE3 in the registry — so this
file iterates the single primitive across the same axes
test_blake2{b,s}.py cover.

The sibling test_nonce_sizes.py / test_auth.py files already
include blake3 as the width-256 representative, but only at the
default key_bits and only one nonce_bits per axis. This file
dedicates the full matrix coverage to blake3 — every (nonce_bits,
mac, key_bits, plaintext_size) combination — so any regression in
the ZMM-batched chain-absorb kernel that depends on a specific
buf shape (20 / 36 / 68 byte input) surfaces.

Run from repo root after building libitb.so:

    go build -trimpath -buildmode=c-shared -o dist/linux-amd64/libitb.so ./cmd/cshared
    python -m pytest bindings/python/tests/test_blake3.py
"""
...
# (hash, ITB_seed_width) — BLAKE3 ships only at -256. The width
...
class TestBLAKE3RoundtripAcrossNonceSizes(unittest.TestCase):
...
class TestBLAKE3TripleRoundtripAcrossNonceSizes(unittest.TestCase):
...
class TestBLAKE3AuthAcrossNonceSizes(unittest.TestCase):
    """Single + Auth round trip + tamper rejection over blake3 ×
    all three nonce sizes. Each MAC primitive is paired with blake3
...
class TestBLAKE3TripleAuthAcrossNonceSizes(unittest.TestCase):
...
class TestBLAKE3PersistenceAcrossNonceSizes(unittest.TestCase):
    """Encrypt with blake3 → snapshot (components + hash_key) → free
    seeds → reconstruct via Seed.from_components → decrypt → verify
    plaintext is bit-identical. Each (key_bits, nonce_bits)
    combination must roundtrip — a regression in the BLAKE3 ASM
    kernel that drifts between original and restored seeds (e.g. a
    mis-seeded KEY broadcast) would surface as a decrypt mismatch
...
class TestBLAKE3RoundtripSizes(unittest.TestCase):
    ...
    fallback, and the test surfaces any boundary bug where the
    batched arm runs on incomplete lane data."""
