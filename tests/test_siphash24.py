"""SipHash-2-4-focused Python binding coverage.

W128 counterpart to test_aescmac.py: the same coverage shape
(nonce-size sweep, single + triple roundtrip, single + triple auth
with tamper rejection, persistence sweep, plaintext-size sweep)
applied to siphash24. SipHash ships at a single width (-128) and
has no internal fixed key — its keying material is the per-call
seed components — so EXPECTED_KEY_LEN["siphash24"] = 0 and the
persistence path stores empty bytes for the hash key.

The sibling test_nonce_sizes.py / test_auth.py files already
include siphash24 as a width-128 representative, but only at the
default key_bits and only one nonce_bits per axis. This file
dedicates the full matrix coverage to siphash24 — every
(nonce_bits, mac, key_bits, plaintext_size) combination — so any
regression in the ZMM-batched ARX chain-absorb kernel that depends
on a specific buf shape (20 / 36 / 68 byte input) surfaces. The
20-byte path runs 14 SipRounds total per pixel, 36-byte runs 18,
and 68-byte runs 26; each is exercised by the Python FFI surface
here.

Run from repo root after building libitb.so:

    go build -trimpath -buildmode=c-shared -o dist/linux-amd64/libitb.so ./cmd/cshared
    python -m pytest bindings/python/tests/test_siphash24.py
"""
...
class TestSipHash24RoundtripAcrossNonceSizes(unittest.TestCase):
...
class TestSipHash24TripleRoundtripAcrossNonceSizes(unittest.TestCase):
...
class TestSipHash24AuthAcrossNonceSizes(unittest.TestCase):
...
class TestSipHash24TripleAuthAcrossNonceSizes(unittest.TestCase):
...
class TestSipHash24PersistenceAcrossNonceSizes(unittest.TestCase):
    """Encrypt with siphash24 → snapshot (components only — SipHash
    has no fixed key) → free seeds → reconstruct via
    Seed.from_components → decrypt → verify plaintext is bit-
    identical. A regression in the SipHash ASM kernel that drifts
    between original and restored seeds (e.g. a wrong constant fold
    or a mis-broadcast lenTag) would surface as a decrypt mismatch
    on the day-2 path while leaving the day-1 encrypt path silent.
    """
...
class TestSipHash24RoundtripSizes(unittest.TestCase):
