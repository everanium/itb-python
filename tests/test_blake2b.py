"""BLAKE2b-focused Python binding coverage.

The shipped Python bindings call into libitb.so via the C ABI; once
the BLAKE2b ZMM-batched chain-absorb kernels land in
hashes/internal/blake2basm and propagate through hashes.Make256Pair /
hashes.Make512Pair, every Python encrypt/decrypt against blake2b256
or blake2b512 routes through the four-pixel-batched dispatch
automatically. The tests below pin down that dispatch contract from
the Python side, mirroring the structural depth that the existing
test_nonce_sizes / test_auth / test_persistence files give to other
primitives.

What this file adds beyond the parametrised iterators in the
sibling test files:

  * blake2b256 coverage across all three SetNonceBits values (the
    sibling test_nonce_sizes file iterates blake2b512 only, since
    one width-256 representative — blake3 — was deemed sufficient).
  * blake2b256 + blake2b512 authenticated round-trip including
    tamper rejection at the dynamic header offset (sibling
    test_auth picks blake2b512 only as the width-512 representative).
  * blake2b256 + blake2b512 persistence round-trip exercising every
    valid (key_bits, nonce_bits) combination — the BLAKE2b ASM
    kernels are length-specialised at 20 / 36 / 68 byte buf shapes,
    so cycling nonce_bits across all three values inside a single
    persistence test surfaces any drift between encrypt-time and
    decrypt-time nonce_bits configuration.

Run from repo root after building libitb.so:

    go build -trimpath -buildmode=c-shared -o dist/linux-amd64/libitb.so ./cmd/cshared
    python -m pytest bindings/python/tests/test_blake2b.py
"""
...
# (hash, ITB_seed_width) — both BLAKE2b widths. The width feeds into
# Seed.from_components key-validation later in this file.
BLAKE2B_HASHES = [
    ("blake2b256", 256),
    ("blake2b512", 512),
]
...
# contract that BLAKE2b256 carries a 32-byte fixed key and BLAKE2b512
# carries a 64-byte fixed key. Mirrors EXPECTED_HASH_KEY_LEN in
# test_persistence.py.
EXPECTED_KEY_LEN = {
    "blake2b256": 32,
    "blake2b512": 64,
...
class TestBLAKE2bRoundtripAcrossNonceSizes(unittest.TestCase):
    """Single-Ouroboros encrypt/decrypt over blake2b256 + blake2b512
    × all three nonce sizes. The four-pixel-batched ASM kernel runs
...
class TestBLAKE2bTripleRoundtripAcrossNonceSizes(unittest.TestCase):
...
class TestBLAKE2bAuthAcrossNonceSizes(unittest.TestCase):
    """Single + Auth round trip + tamper rejection over blake2b256 +
    blake2b512 × all three nonce sizes. Each MAC primitive is paired
    with each BLAKE2b width to confirm the auth scaffolding is
    width-agnostic."""
...
class TestBLAKE2bTripleAuthAcrossNonceSizes(unittest.TestCase):
...
class TestBLAKE2bPersistenceAcrossNonceSizes(unittest.TestCase):
    """Encrypt with blake2b → snapshot (components + hash_key) → free
    seeds → reconstruct via Seed.from_components → decrypt → verify
    plaintext is bit-identical. Each (hash, key_bits, nonce_bits)
    combination must roundtrip — a regression in the BLAKE2b ASM
    kernel that drifts between original and restored seeds (e.g. a
    mis-seeded h0 broadcast) would surface as a decrypt mismatch on
...
class TestBLAKE2bRoundtripSizes(unittest.TestCase):
