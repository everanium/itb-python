"""AES-CMAC-focused Python binding coverage.

W128 counterpart to test_chacha20.py / test_blake3.py: the same
coverage shape (nonce-size sweep, single + triple roundtrip,
single + triple auth with tamper rejection, persistence sweep,
plaintext-size sweep) applied to aescmac. AES-CMAC ships at a
single width (-128 — the AES block size) so this file iterates
the single primitive across the same axes test_chacha20.py covers.

The sibling test_nonce_sizes.py / test_auth.py files already
include aescmac as a width-128 representative, but only at the
default key_bits and only one nonce_bits per axis. This file
dedicates the full matrix coverage to aescmac — every
(nonce_bits, mac, key_bits, plaintext_size) combination — so any
regression in the ZMM-batched VAES chain-absorb kernel that
depends on a specific buf shape (20 / 36 / 68 byte input) surfaces.
The 20-byte path runs 2 AES rounds, 36-byte runs 3, and 68-byte
runs 5; each is exercised by the Python FFI surface here.

Run from repo root after building libitb.so:

    go build -trimpath -buildmode=c-shared -o dist/linux-amd64/libitb.so ./cmd/cshared
    python -m pytest bindings/python/tests/test_aescmac.py
"""
...
# block size). The width feeds into Seed.from_components key
...
class TestAESCMACRoundtripAcrossNonceSizes(unittest.TestCase):
...
class TestAESCMACTripleRoundtripAcrossNonceSizes(unittest.TestCase):
...
class TestAESCMACAuthAcrossNonceSizes(unittest.TestCase):
    """Single + Auth round trip + tamper rejection over aescmac ×
    all three nonce sizes. Each MAC primitive is paired with
    aescmac to confirm the auth scaffolding is width-agnostic."""
...
class TestAESCMACTripleAuthAcrossNonceSizes(unittest.TestCase):
...
class TestAESCMACPersistenceAcrossNonceSizes(unittest.TestCase):
    """Encrypt with aescmac → snapshot (components + hash_key) →
    free seeds → reconstruct via Seed.from_components → decrypt →
    verify plaintext is bit-identical. Each (key_bits, nonce_bits)
    combination must roundtrip — a regression in the AES-CMAC ASM
    kernel that drifts between original and restored seeds (e.g. a
    mis-broadcast AES round key or a wrong lenTag fold) would
    surface as a decrypt mismatch on the day-2 path while leaving
    the day-1 encrypt path silent.
    """
...
class TestAESCMACRoundtripSizes(unittest.TestCase):
    """Roundtrip aescmac on plaintext sizes that span multiple
    chunk boundaries. ITB's processChunk128 batches 4 pixels per
    BatchHash call; trailing partial batches must dispatch via the
    per-lane fallback, and the test surfaces any boundary bug where
    the batched arm runs on incomplete lane data."""
