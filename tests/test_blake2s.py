"""BLAKE2s-focused Python binding coverage.
...
applied to blake2s. BLAKE2s ships at a single width (-256) — there is
no -224 / -128 BLAKE2s in the registry — so this file iterates the
single primitive across the same axes test_blake2b.py covers for
both BLAKE2b widths.

What this file adds beyond the parametrised iterators in the sibling
test files:

  * blake2s coverage across all three SetNonceBits values (sibling
    test_nonce_sizes iterates blake3 + blake2b512 + siphash24 — no
    BLAKE2s representative at the width-256 axis).
  * blake2s authenticated round-trip including tamper rejection at
    the dynamic header offset (sibling test_auth picks blake3 +
    blake2b512 + siphash24).
  * blake2s persistence round-trip cycling nonce_bits across all
    three values inside a single test — the BLAKE2s ASM kernels are
    length-specialised at 20 / 36 / 68 byte buf shapes, so the full
    sweep surfaces any drift between encrypt-time and decrypt-time
    nonce_bits configuration.

Run from repo root after building libitb.so:

    go build -trimpath -buildmode=c-shared -o dist/linux-amd64/libitb.so ./cmd/cshared
    python -m pytest bindings/python/tests/test_blake2s.py
"""
...
# (hash, ITB_seed_width) — BLAKE2s ships only at -256. The width feeds
...
# contract that BLAKE2s carries a 32-byte fixed key. Mirrors
...
class TestBLAKE2sRoundtripAcrossNonceSizes(unittest.TestCase):
    """Single-Ouroboros encrypt/decrypt over blake2s × all three nonce
    sizes. The four-pixel-batched ASM kernel runs a length-specialised
    path at 20 / 36 / 68 bytes; the loop below drives every shape
...
class TestBLAKE2sTripleRoundtripAcrossNonceSizes(unittest.TestCase):
...
class TestBLAKE2sAuthAcrossNonceSizes(unittest.TestCase):
    """Single + Auth round trip + tamper rejection over blake2s × all
    three nonce sizes. Each MAC primitive is paired with blake2s to
...
class TestBLAKE2sTripleAuthAcrossNonceSizes(unittest.TestCase):
...
class TestBLAKE2sPersistenceAcrossNonceSizes(unittest.TestCase):
    """Encrypt with blake2s → snapshot (components + hash_key) → free
    seeds → reconstruct via Seed.from_components → decrypt → verify
    plaintext is bit-identical. Each (hash, key_bits, nonce_bits)
    combination must roundtrip — a regression in the BLAKE2s ASM
...
class TestBLAKE2sRoundtripSizes(unittest.TestCase):
    ...
    fallback, and the test surfaces any boundary bug where the batched
    arm runs on incomplete lane data."""
